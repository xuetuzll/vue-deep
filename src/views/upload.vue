<template>
  <div>
    <Upload ref="upload" :action="`${baseURL}/upload_file`" multiple :before-upload="beforeUpload"
...
export default {
          ...
          render: (h, {row}) => {
            return (
              <span>
                ...
                <i-button on-click={this.deleteFile.bind(this. row.key)}>删除按钮</i-button>
              </span>
            )
          }
        }
  ...
  methods: {
    ...
    updateFileList (){
      //更新视图比较直接的方法
      //找到删除文件的key，遍历列表删除key所在的文件，视图就会更新
